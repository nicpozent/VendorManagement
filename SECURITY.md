...
| Personal data stored in clear | **AES-256-GCM at-rest encryption** (`Encryption:Enabled` + base64 32-byte `Encryption:Key`) of vendor contacts, imported-user identity, review NDA/owner emails, and **evidence-attachment bytes** (signed NDAs, SOC 2 / ISO / DPA / pen-test reports) — envelope-marked (`enc:v1:` for text, `ENC1` magic for blobs), key-rotatable, transparent read/write. |
| Cross-user access to a review (IDOR) | **Owner-or-leadership check on every review surface** — read, update, finish, delete, memo export, intake scan, reminder mail, attachments, history, **Archive (list + memo snapshot)** and **Compare** all enforce the same visibility rule; managers can only reach their own reviews, leadership the whole portfolio. |
| Clickjacking / MIME sniffing | **Baseline security response headers** on every API response — `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Content-Security-Policy: frame-ancestors 'none'`, `Referrer-Policy: no-referrer`; attachment downloads are served `Content-Disposition: attachment`. |
| HTML injection into outbound mail | Vendor-controlled fields (contact/vendor name, verdict reason) are **HTML-encoded** before interpolation into NDA/reminder mail bodies; oversize uploads are rejected before buffering (413) in addition to the 15 MB post-read cap. |
